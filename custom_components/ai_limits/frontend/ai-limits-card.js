  ...
  render() {
    ...
    Object.keys(states).forEach((entityId) => {
      ...
      if (entityId.startsWith("sensor.claude_subscription_")) {
        ...
        account = parts[0];
        metric = parts.slice(1).join("_");
      } else if (entityId.startsWith("binary_sensor.devin_")) {
        provider = "Devin";
        const parts = entityId.replace("binary_sensor.devin_", "").split("_");
        account = parts[0];
        metric = parts.slice(1).join("_");
      }
    ...
    if (keys.length === 0) {
      ...
      keys.forEach((key) => {
        ...
        const isDevin = group.provider === "Devin";
        ...
        if (isClaude) {
          ...
          logoHtml = `<svg class="brand-logo" viewBox="0 0 24 24" style="fill: #0CC80A;"><path d="M19,2H5A2,2 0 0,0 3,4V18A2,2 0 0,0 5,20H9V22H11V20H13V22H15V20H19A2,2 0 0,0 21,18V4A2,2 0 0,0 19,2M19,18H5V4H19V18M7.5,7.5A1.5,1.5 0 1,1 6,9A1.5,1.5 0 0,1 7.5,7.5M16.5,7.5A1.5,1.5 0 1,1 15,9A1.5,1.5 0 0,1 16.5,7.5M16,13V15H8V13H16Z"/></svg>`;
        }
